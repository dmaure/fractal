...
describe('ServerValidator', () => {
  ...
  describe('validatePorts', () => {
    it('debe aprobar si los puertos 80 y 443 están disponibles', async () => {
      vi.spyOn(mockClient, 'executeCommand')
        .mockResolvedValue({
          success: true,
          stdout: '', // Sin output = puerto libre
          stderr: '',
          exitCode: 0,
        } as SshCommandResult);
    ...
    it('debe detectar puertos ocupados', async () => {
      vi.spyOn(mockClient, 'executeCommand')
        // Puerto 80 ocupado
        .mockResolvedValueOnce({
          success: true,
          stdout: 'tcp   LISTEN 0      511          0.0.0.0:80',
          stderr: '',
          exitCode: 0,
        } as SshCommandResult)
        // Proceso en puerto 80
        .mockResolvedValueOnce({
          success: true,
          stdout: 'nginx',
          stderr: '',
      ...
      expect(result.occupiedPorts).toContain('80 (nginx)');
    });
  });
});
